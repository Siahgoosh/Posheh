...
import { createPreviewUrl, revokePreviewUrl } from '../utils/imageCompression'
...
async function validateSceneImage(file: File): Promise<{ valid: boolean; errors: string[] }> {
  ...
  if (file.size > MAX_MB * 1024 * 1024) {
    errors.push(`حداکثر حجم ${MAX_MB} مگابایت است.`)
  }

  const bitmap = await createImageBitmap(file).catch(() => null)
  if (bitmap) {
    if (bitmap.width > MAX_DIM || bitmap.height > MAX_DIM) {
      errors.push(`حداکثر اندازه ${MAX_DIM} پیکسل است.`)
    }
    bitmap.close()
  }
...
export function SceneImageUploader({ tourId, onUploaded }: Props) {
  ...
  const processFile = useCallback(async (file: File, taskId: string) => {
    const validation = await validateSceneImage(file)
    if (!validation.valid) {
    ...
    try {
      ...
      await tourApi.uploadSceneImage(tourId, file, {
      ...
      updateUploadTask(taskId, { status: 'error', error: extractApiError(err, 'آپلود ناموفق بود.') })
    }
  ...
  const enqueueFiles = useCallback((files: FileList | File[]) => {
    Array.from(files).forEach((file) => {
      ...
      addUploadTask({ id, file, name: file.name, progress: 0, status: 'pending', previewUrl })
      processFile(file, id)
    })
    ...
    <div className="space-y-3">
      <div
        ...
        className={`relative rounded-xl border-2 border-dashed p-6 text-center cursor-pointer transition-all duration-300 ${
          dragOver ? 'border-primary bg-primary/10 scale-[1.01]' : 'border-card-border hover:border-primary/40 hover:bg-primary/5'
        }`}
      >
        <Upload className="h-8 w-8 mx-auto mb-2 text-primary/70" />
        <p className="text-sm font-medium">عکس‌های موبایل را اینجا رها کنید</p>
        <p className="text-xs text-muted mt-1">Smart Walk — JPEG/PNG/WebP/AVIF تا {MAX_MB}MB</p>
        <input
          ref={inputRef}
          type="file"
      ...
      {uploadTasks.length > 0 && (
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {uploadTasks.map((task) => (
            <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-black/20 border border-card-border/50">
              {task.previewUrl ? (
