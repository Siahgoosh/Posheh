...
export function buildHotspotMarkerHtml(hotspot: TourHotspot, brandColor = '#2dd4bf'): string {
  ...
  const safeTitle = escapeHtml(String(hotspot.tooltip || label || ''))
  ...
  return `
    ...
    ${label && style.label !== '' ? `<div class="vt-hotspot-label" style="margin-top:4px;font-size:10px;color:white;text-shadow:0 1px 3px black;text-align:center;white-space:nowrap;">${safeLabel}</div>` : ''}
  `
}
