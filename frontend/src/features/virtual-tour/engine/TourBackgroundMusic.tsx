...
interface Props {
  ...
  editorMode?: boolean
  /** When true, background music plays in the editor for preview (lower volume). */
 ...
 */
export function TourBackgroundMusic({
  musicUrl,
  editorMode = false,
  ...
  const allowPlayback = Boolean(musicUrl) && (!editorMode || previewInEditor)
  const playbackVolume = editorMode && previewInEditor ? Math.min(volume, 0.18) : volume

  const tryPlay = useCallback(() => {
    const el = audioRef.current
    if (!el || !musicUrl || !allowPlayback) return
    el.play()
      .then(() => {
        setBlocked(false)
        setPlaying(true)
  ...
  }, [musicUrl, allowPlayback])

  useEffect(() => {
    const el = audioRef.current
    if (!el || !allowPlayback || !musicUrl) {
      setPlaying(false)
      setBlocked(false)
      if (el) {
        el.pause()
    ...
    const resolved = musicUrl.startsWith('http')
      ? musicUrl
      : `${window.location.origin}${musicUrl.startsWith('/') ? '' : '/'}${musicUrl}`
    if (el.src !== resolved) {
      el.src = resolved
      el.load()
    }
  ...
  }, [musicUrl, allowPlayback, playbackVolume, tryPlay])

  useEffect(() => {
    if (!allowPlayback || !musicUrl) return
  ...
  }, [allowPlayback, musicUrl, tryPlay])

  if (!allowPlayback || !musicUrl) return null
    ...
    <>
      ...
      {blocked && !playing && (
        ...
        >
          ...
          <span>{editorMode ? 'پیش‌نمایش موزیک' : 'پخش موزیک تور'}</span>
