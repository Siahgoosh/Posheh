...
export function FloatingChatWidget() {
  ...
  const lastSeenOperatorIdRef = useRef(0)
  ...
  // Poll while chat is open to refresh the thread.
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        setMessages(list)
        const maxOpId = list
          .filter((m) => m.sender_type === 'operator')
          .reduce((max, m) => Math.max(max, m.id), 0)
        if (maxOpId > lastSeenOperatorIdRef.current) {
          lastSeenOperatorIdRef.current = maxOpId
        }
        setUnread(0)
      } catch { /* ignore */ }
    }
  ...
  // Poll while closed so unread badge updates for operator replies.
  useEffect(() => {
    if (open || !conversationUuid || !visitorToken) return
    const poll = async () => {
      try {
        const res = await communicationApi.getMessages(conversationUuid, visitorToken)
        const list = res.data.data as Message[]
        const newOps = list.filter(
          (m) => m.sender_type === 'operator' && m.id > lastSeenOperatorIdRef.current,
        )
        setUnread(newOps.length)
      } catch { /* ignore */ }
    }
    poll()
    const t = setInterval(poll, 8000)
    return () => clearInterval(t)
  }, [open, conversationUuid, visitorToken])
