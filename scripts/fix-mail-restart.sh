#!/bin/bash
# Fix Mailu admin/webmail restart loop — never deletes or recreates posheh_mailu network.
set -euo pipefail
...
log "1/4 Ensuring mailu network + syncing SUBNET..."
"$ROOT/scripts/ensure-mailu-network.sh"

log "2/4 Patching mailu.env..."
...
ADMIN_STATE=$($COMPOSE ps mailu-admin --format '{{.State}}' 2>/dev/null || echo missing)
WEBMAIL_STATE=$($COMPOSE ps mailu-webmail --format '{{.State}}' 2>/dev/null || echo missing)
NEEDS_RECREATE=0
if echo "$ADMIN_STATE$WEBMAIL_STATE" | grep -qi restarting; then
  NEEDS_RECREATE=1
  log "Admin/webmail restarting — will force-recreate mail services only"
fi

log "3/4 Starting Mailu..."
...
log "4/4 Connecting nginx + app to mailu network..."
