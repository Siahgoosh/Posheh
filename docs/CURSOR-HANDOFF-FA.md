...
> **نسخه سند:** ۱۴۰۴/۰۵/۲۹ (۲۰۲۶-۰۸-۰۳) — به‌روزرسانی فایلینگ و UI  
...
**شاخه `main` روی production احتمالاً عقب‌تر از آخرین کارهاست.** بسیاری از قابلیت‌های جدید در PRهای Draft روی شاخه‌های `cursor/*-e117` هستند و هنوز merge نشده‌اند. قبل از هر deploy جدی، PRهای #43 تا #46 را بررسی و در صورت آماده بودن merge کن.
...
## پیوست ب: تغییرات اخیر (۲۰۲۶-۰۸-۰۳)

### فایلینگ (اولویت بالا — رفع باگ ثبت فایل)

| مشکل | رفع |
|------|-----|
| خطای «همه فیلدها را پر کنید» با وجود پر بودن | `buildFilingPayload()` فعال شد — فیلدهای nested در `filing_data` درست ارسال می‌شوند |
| تاریخ انقضا دستی | `JalaliDatePicker` — انتخاب روز/ماه/سال شمسی |
| مشاور مسئول دستی | `user_select` — لیست از `/office/team` |
| نام مالک در وبسایت | `PublicPropertyResource` — حذف `owner_name` و `filing_data.owner` از API عمومی |
| پیام خطای انگلیسی | پیام‌های فارسی در `StorePropertyRequest` |

**فایل‌های کلیدی:**
- `frontend/src/components/ui/JalaliDatePicker.tsx`
- `frontend/src/components/filing/DynamicFilingFields.tsx`
- `frontend/src/lib/filing.ts` — `buildFilingPayload`, `validateFilingForm`
- `backend/app/Http/Resources/PublicPropertyResource.php`

### ثبت‌نام
- آپلود لوگو برای **همه پلن‌ها** (شامل solo) با `LogoUploader` drag-and-drop

### UI/تم
- تم تیره جدید در `frontend/src/index.css` (گرادیان، glass، card-shine)
- دستیار محتوا: hero، کارت‌های highlight، انتخاب لحن با emoji

### نسخه اپ
- موبایل: `1.1.0+8` در `mobile/pubspec.yaml`
- PWA: `manifest.json` به‌روز
- بیلد اندروید/ویندوز: `./scripts/build-releases.sh` (نیاز به Flutter SDK — CI در GitHub Actions)

```bash
# بیلد روی سرور یا ماشین با Flutter
./scripts/build-releases.sh
# خروجی: frontend/public/downloads/posheh-android.apk
```
