...
{
    /** @return array{ok: bool, message_fa: string, ctas?: int, rules?: int, goals?: int} */
    public function ensureDefaults(): array
    {
        if (! Schema::hasTable('cro_ctas') || ! Schema::hasTable('cro_cta_rules') || ! Schema::hasTable('cro_page_goals')) {
            return [
                'ok' => false,
                'message_fa' => 'جداول تبدیل/سرنخ موجود نیست — ابتدا migrate را اجرا کنید.',
            ];
        }
        ...
        try {
            ...
            return [
                'ok' => true,
                'message_fa' => 'فراخوان‌ها، قوانین و اهداف صفحه آماده شدند.',
                'ctas' => CroCta::count(),
                'rules' => CroCtaRule::count(),
                'goals' => CroPageGoal::count(),
            ];
        } catch (\Throwable $e) {
            return [
                'ok' => false,
                'message_fa' => 'خطا در راه‌اندازی تبدیل/سرنخ: '.$e->getMessage(),
            ];
        }
    }
}
